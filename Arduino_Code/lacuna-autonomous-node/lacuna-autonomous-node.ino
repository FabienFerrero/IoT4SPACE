 ...
 *
 * Description: Lacuna autonomous node sketch.
 * 
...
void periodic_check() {
  ...
  if (fix) {
      ...
      uint32_t start;
      uint32_t duration;
      uint16_t maxelevation = predictpass(gnss_lat, gnss_lon, gnss_alt, &start, &duration, &rxtx);
...
uint16_t predictpass(float latitude, float longitude, float altitude, uint32_t* start, uint32_t* duration, bool* rxtx){
