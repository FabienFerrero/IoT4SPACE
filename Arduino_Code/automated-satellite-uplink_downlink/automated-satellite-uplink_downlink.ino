 ...
 * 
 * From Expermental code Sat node met Antenna switch Andries  18-03-2020
 * 
 * Experimental code with Rx LS3 and multi sat wake-up Fabien 07-02-2021
 */
...
#define MINELEVATION 16       // Test also low orbit passages

// max. 250 seconds for GPS fix
#define FIXTIME 180

// Define duration of LS3 RX
#define LS3RX 360
...
//#define LS2C // Activate LS2C
//#define FAKEPASS
//#define DEBUGSLEEP   //Comment out for production
#define OFFSET 100 // additional time offset in ms to  limit interference for multiple devices
#define BEACON_MODE 1 //Activate beacon reception
#define LS3_UP 868500000; // uplink freq for LS3 measurement, must be a loraWan channel
...
#elif LACUNA_TEST == 9
// Design kit transparent 5cm
static byte networkKey[] = { 0xDD, 0x42, 0xA0, 0xDF, 0x11, 0xCC, 0x35, 0x90, 0xC8, 0xAF, 0xF5, 0x4F, 0x5D, 0x2A, 0xD0, 0x65 };
static byte appKey[] = { 0x51, 0x81, 0x1A, 0x3E, 0xED, 0x12, 0x19, 0x97, 0xF2, 0x2B, 0xF9, 0x1C, 0xD6, 0x07, 0xA5, 0xDA };
static byte deviceAddress[] = { 0x26, 0x01, 0x10, 0x8C };
#elif LACUNA_TEST == 10
// Design kit purple 15cm
static byte networkKey[] = { 0x12, 0x0A, 0xEB, 0x11, 0x01, 0xB7, 0x63, 0xF1, 0x49, 0xED, 0x6A, 0x11, 0x40, 0x4C, 0xB2, 0x42 };
static byte appKey[] = { 0x5C, 0xA5, 0x04, 0x42, 0x61, 0x29, 0xE4, 0x67, 0xBE, 0xCE, 0x27, 0x52, 0xA9, 0x64, 0x55, 0xE2 };
static byte deviceAddress[] = { 0x26, 0x01, 0x13, 0x01 };
#elif LACUNA_TEST == 11
// Design kit ABS 0cm
static byte networkKey[] = { 0xD1, 0xA9, 0xB2, 0x50, 0xA8, 0x68, 0xD2, 0xCF, 0x70, 0x3E, 0x8B, 0x73, 0x03, 0x3C, 0xD2, 0xAE };
static byte appKey[] = { 0x7C, 0x6B, 0xAC, 0x19, 0x44, 0x73, 0xC2, 0x3F, 0x31, 0xB1, 0x03, 0x34, 0xA4, 0x41, 0x39, 0x9D };
static byte deviceAddress[] = { 0x26, 0x01, 0x3D, 0x5E };
#elif LACUNA_TEST == 12
// Design kit ABS - AA - Sent to Thomas
static byte networkKey[] = { 0x58, 0x93, 0x45, 0xF7, 0xF0, 0x49, 0x0C, 0x97, 0xD2, 0xA2, 0xF5, 0xC2, 0x71, 0x8E, 0x25, 0x23 };
static byte appKey[] = { 0x6D, 0x19, 0x5D, 0xC1, 0x76, 0x84, 0xE3, 0x53, 0xD8, 0xDC, 0x98, 0x23, 0x29, 0xD8, 0x13, 0xA2 };
static byte deviceAddress[] = { 0x26, 0x01, 0x39, 0xD1 };

#elif LACUNA_TEST == 13
// Design kit Purple N°2 booster
static byte networkKey[] = { 0xFF, 0xB2, 0x3E, 0x50, 0x44, 0xE5, 0x51, 0x2E, 0xAA, 0x1A, 0xAD, 0xC2, 0xFD, 0x11, 0x52, 0x21 };
static byte appKey[] = { 0x60, 0x13, 0xC4, 0x33, 0xFA, 0x33, 0x61, 0x2A, 0x26, 0x9E, 0x36, 0x56, 0xC1, 0x4A, 0xAC, 0xAD };
static byte deviceAddress[] = { 0x26, 0x01, 0x1E, 0xB9 };

#elif LACUNA_TEST == 14
// Design kit ABS N°2 20cm
static byte networkKey[] = { 0x19, 0xD1, 0x96, 0x8F, 0x5C, 0x54, 0xB2, 0x86, 0x38, 0x9F, 0xDE, 0x5B, 0xF7, 0x67, 0xF4, 0xBA };
static byte appKey[] = { 0xA2, 0x87, 0xBA, 0x58, 0x10, 0x2A, 0x55, 0x58, 0x8A, 0xA0, 0xB8, 0xF1, 0x29, 0x43, 0xE6, 0x18 };
static byte deviceAddress[] = { 0x26, 0x01, 0x1A, 0x5D };

#elif LACUNA_TEST == 15
// Design kit Transp N°2 10cm
static byte networkKey[] = { 0xE4, 0x5E, 0xD8, 0x53, 0xCE, 0x47, 0x86, 0xE1, 0x63, 0x56, 0xBD, 0x2B, 0x46, 0xF1, 0xE2, 0x3C };
static byte appKey[] = { 0x95, 0xE3, 0x23, 0x82, 0x02, 0xA3, 0xCC, 0x4E, 0xB0, 0x09, 0x10, 0x9A, 0x3B, 0x97, 0x7E, 0x9D };
static byte deviceAddress[] = { 0x26, 0x01, 0x3E, 0x8B };
...
char satname[] = "LS2d";
uint32_t tle_epoch;
uint8_t  tle_age = 0x07; // TLE are updated in the code, so TLE is 0
...
//  or global change - replace Serial <-> Serial3 to switch debugchannel
  ...
  Serial.print(" Satellite Transmission : ");
  ...
  if (wakeupreason == 0) {
    ...
    byte fix = updategps();
      ...
      if (fix) {
          ...
          uint32_t start;
          uint32_t start1 = 0xFFFFFFFF; // set max value
          uint32_t start2 = 0xFFFFFFFF; // set max value
          uint32_t start3 = 0xFFFFFFFF; // set max value
          uint32_t duration;
          uint32_t duration1;
          uint32_t duration2;
          uint32_t duration3;
          initialize_tle(1);   // start with initializing satellite 1
          uint16_t maxelevation = predictpass(gnss_lat, gnss_lon, gnss_alt, &start, &duration,1);
          
          initialize_tle(2);   // try satellite 2
          uint16_t maxelevation2 = predictpass(gnss_lat, gnss_lon, gnss_alt, &start2, &duration2,2);
          
      #if LS2C      // If LS2C is available
          initialize_tle(3);   // try satellite 2
          uint16_t maxelevation3 = predictpass(gnss_lat, gnss_lon, gnss_alt, &start3, &duration3,3);
      #endif
          
          nextsat=1; //  nextsat is set to 1 (LS1)
          if (start2<start) {
            start=start2;
            duration=duration2;
            initialize_tle(2);   // Sat 2 is the first one to come
            nextsat=2;
            }
          if (start3<start) {
            start=start3;
            duration=duration3;
            initialize_tle(3);   // Sat 3 is the first one to come
            nextsat=3;
            }          
      
          if (maxelevation != 0) {
    
            Serial.print("  Next pass set on Sat N°");
            Serial.print(nextsat);
            Serial.print(" with max. elevation: ");
...
// Define if it is a Tx or Rx pass

unixtime = RTC.getEpoch();
int LS2DLS3 = floor(unixtime/86400.0);

 if (nextsat == 2 && (LS2DLS3 % 2 == 1) && BEACON_MODE) {   // LS3 mode  days are odd

nextsat = 4 ; // Degine number for LS3
...
// transmission parameters for terrestrial LoRa
  ...
  txParams.frequency = 862800000;
  txParams.preambleLength = 488;
  txParams.ldro= true; // don't work in debug mode if enable
  txParams.rxTimeout = 10000; // in ms


// Receive during LS3RX
 long startTime = millis();     
   while (millis() - startTime < (LS3RX*1000))
   {
...
Serial.println("Waiting for LoRa message"); 
  uint32_t bytes = lsReceiveLoraSat(&loraWANParams, &txParams, (byte *)payload);
  int snr;
  int rssi;
  int signalrssi;
  int frequency_error;
  ...
  if (bytes) {
    ...
    snr = txParams.snr;
    rssi = txParams.rssi;
    signalrssi = txParams.signalrssi;
    frequency_error = txParams.frequency_error;
...
Serial.println("Sending LoRa message"); 
  ...
  delay (5000); // Packet sequence are sent every 10s, we delay by by 5s to be ready for the next sequence  

// Receive parameters for LS3
  lsCreateDefaultLoraTxParams(&txParams);
  txParams.spreadingFactor = lsLoraSpreadingFactor_10;
  txParams.bandwidth = lsLoraBandwidth_500_khz;
  txParams.frequency = 862800000;
  ...
  txParams.rxTimeout = 10000; // in ms
    ...
    // Set up back default transmission parameters for terrestrial LoRa
  lsCreateDefaultLoraTxParams(&txParams);
  loraWANParams.rxEnable = true;
...
// Please update manually TLE
  
 // TLE data sat1                  
  char tle_ls1_1[] = "1 44109U 19018AF  21041.68972496  .00002056  00000-0  65326-4 0  9995";
  char tle_ls1_2[] = "2 44109  97.4394 118.3092 0058508  84.9318 275.8598 15.32182485104041"; 
  
 // TLE data sat2                   
  char tle_ls2d_1[] = "1 46492U 20068G   21041.78325958  .00000882  00000-0  68715-4 0  9995";
  char tle_ls2d_2[] = "2 46492  97.6821 340.1828 0019221 142.1480 248.1497 15.03675053 20306";   


  // TLE data sat3                   
  char tle_ls2c_1[] = "1 46913U 20081J   21035.08031306  .00000325  00000-0  32442-4 0  9995";
  char tle_ls2c_2[] = "2 46913  36.9043 295.0546 0010113 334.1414  25.8821 14.99282826 12625";   
  ...
  switch (satnumber) {
     ...
     break;
     case 3:  
     LacunaSat.init(satname,tle_ls2c_1,tle_ls2c_2);     //initialize satellite parameters from firmware
     break;
     case 2:
...
uint16_t predictpass(float latitude, float longitude, float altitude, uint32_t* start, uint32_t* duration, int sat){
    ...
    if ( error == 1) { //no error, prints overpass information
        ...
        Serial.println("  Sat N° " + String(sat));
...
#ifdef FAKEPASS
        *start = RTC.getEpoch()+40; // Normal is +240
        *duration = 30;
#else
...
uint32_t UnixToDhhmmss(uint32_t unixtime) {
  ...
  byte networkKey[] = { 0x94, 0x33, 0xA3, 0xAF, 0xF5, 0x2A, 0x77, 0x4C, 0x90, 0xDD, 0x68, 0x02, 0x17, 0x78, 0x63, 0xA4 };
  byte appKey[] = { 0x25, 0xC0, 0x73, 0x48, 0x2F, 0xAD, 0x67, 0x95, 0x6E, 0xD0, 0x56, 0x43, 0x27, 0xCA, 0x3A, 0x72 };
  byte deviceAddress[] = { 0x26, 0x01, 0x30, 0x8A };
